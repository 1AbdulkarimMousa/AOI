  ...
  <section x-show="collectMode==='create'" class="collection-layout">
    ...
    <article class="panel collection-form-panel">
      ...
      <form x-show="collectionType==='evidence'" class="research-form" @submit.prevent>
        <div class="form-grid form-grid-three">
          <label><span>Respondent</span><select x-model="researchForms.evidence.respondentId"><option value="">No respondent</option><template x-for="item in researchRespondents" :key="item.id"><option :value="item.id" x-text="item.externalId+' · '+item.segmentName"></option></template></select></label>
          <label><span>Session</span><select x-model="researchForms.evidence.sessionId"><option value="">No session</option><template x-for="item in (data.sessions||[])" :key="item.id"><option :value="item.id" x-text="formatDate(item.sessionDate)+' · '+item.method"></option></template></select></label>
      ...
      <form x-show="collectionType==='observation'" class="research-form" @submit.prevent>
        ...
        <div class="form-grid"><label><span>Respondent</span><select x-model="researchForms.observation.respondentId"><option value="">Aggregate or unlinked</option><template x-for="item in researchRespondents" :key="item.id"><option :value="item.id" x-text="item.externalId"></option></template></select></label><label><span>Source link</span><input type="url" x-model.trim="researchForms.observation.sourceLink" placeholder="https://"></label></div>

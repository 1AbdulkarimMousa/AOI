...
<div x-show="view==='research' && researchTab==='collect'" class="pmf-surface">
  ...
  <section x-show="collectMode==='browse'" class="panel collect-library">
    ...
    <div class="collect-table" role="table" aria-label="Collected research records">
      <div class="collect-table-row collect-table-header" role="row"><span role="columnheader">Record</span><span role="columnheader">Respondent</span><span role="columnheader">Owner</span><span role="columnheader">Workflow</span><span role="columnheader">Updated</span></div>
      <template x-for="record in filteredCollectRecords" :key="record.recordType+'-'+record.id"><button class="collect-table-row" role="row" @click="openCollectRecord(record)"><span role="cell"><small x-text="record.recordType.replaceAll('_',' ')"></small><strong x-text="record.title"></strong><em x-show="record.identityTrail" x-text="record.identityTrail"></em></span><span role="cell"><strong x-text="record.respondentCode||'Unlinked'"></strong><small x-text="record.segmentName||'No segment'"></small></span><span role="cell" x-text="record.ownerName||'Shared contributor'"></span><span role="cell" class="status-badge" :class="'status-'+record.workflowStatus" x-text="record.workflowStatus.replaceAll('_',' ')"></span><span role="cell" x-text="formatDate(record.recordDate)"></span></button></template>
  ...
  <section x-show="collectMode==='create'" class="collection-layout">
    <nav class="collection-rail" aria-label="Collection record type">
      <button :class="collectionType==='respondent' && 'active'" @click="startCollectionRecord('respondent')"><b>01</b><span><strong>Respondent</strong><small>Identity, assignment, consent</small></span></button>
      <button :class="collectionType==='session' && 'active'" @click="startCollectionRecord('session')"><b>02</b><span><strong>Session</strong><small>Behavior and unmet need</small></span></button>
      <button :class="collectionType==='evidence' && 'active'" @click="startCollectionRecord('evidence')"><b>03</b><span><strong>Evidence</strong><small>Claim, source, limitations</small></span></button>
      <button :class="collectionType==='product_event' && 'active'" @click="startCollectionRecord('product_event')"><b>04</b><span><strong>Product event</strong><small>Use, value, friction</small></span></button>
      <button :class="collectionType==='value_exchange' && 'active'" @click="startCollectionRecord('value_exchange')"><b>05</b><span><strong>Value exchange</strong><small>Price and commitment</small></span></button>
      <button :class="collectionType==='observation' && 'active'" @click="startCollectionRecord('observation')"><b>06</b><span><strong>Matrix observation</strong><small>Structured PMF metric</small></span></button>
    </nav>

    <article class="panel collection-form-panel">
      <div class="collection-form-heading"><div><span class="eyebrow" x-text="collectionType.replaceAll('_',' ')"></span><h2 x-text="researchEditState ? 'Edit research record' : 'Research record'"></h2></div><span class="draft-rule">Drafts stay assignment-scoped</span></div>
      <div x-show="researchEditState?.reviewNotes" class="admin-notice"><strong>Revision guidance</strong><span x-text="researchEditState?.reviewNotes"></span></div>
      ...
      <div class="collection-actions"><button class="button button-secondary" :disabled="savingResearch" @click="saveResearchRecord(collectionType,'draft')">Save draft</button><button class="button button-primary" :disabled="savingResearch" @click="saveResearchRecord(collectionType,'submitted')" x-text="savingResearch ? 'Saving…' : (researchEditState?.workflowStatus==='revision_requested' ? 'Resubmit for review' : 'Submit for review')"></button></div>
    </article>
  </section>
  ...
  <div x-show="selectedCollectRecord" class="modal-backdrop drawer-backdrop" @mousedown.self="closeCollectRecord()">
    <aside class="collect-detail-drawer" role="dialog" aria-modal="true" aria-labelledby="collect-detail-title" tabindex="-1" @keydown.escape.stop="closeCollectRecord()" @keydown.tab="trapCollectDetailFocus($event)">
      ...
      <template x-if="collectDetail"><div class="collect-detail-body">
        ...
        <section x-show="canEditSelectedCollectRecord" class="collect-follow-on"><span class="eyebrow">Assigned draft</span><button class="button button-primary" @click="startResearchRevision()">Edit record</button></section>
        <section x-show="selectedCollectRecord?.respondentId||selectedCollectRecord?.recordType==='respondent'" class="collect-follow-on"><span class="eyebrow">Continue collection</span><div><button class="button button-secondary" @click="continueCollection('session')">New session</button><button class="button button-secondary" @click="continueCollection('evidence')">New evidence</button><button class="button button-secondary" @click="continueCollection('product_event')">Product event</button></div></section>
      </div></template>
    </aside>
  </div>
</div>`;

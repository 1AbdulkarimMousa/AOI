...
alter table public.help_center_articles add column if not exists project_id uuid references public.projects(id) on delete cascade;
update public.help_center_articles article
set project_id = (
  select candidate.id from public.projects candidate
  where candidate.organization_id = article.organization_id and candidate.status = 'active'
  order by candidate.created_at limit 1
)
where article.project_id is null;
alter table public.help_center_articles alter column project_id set not null;
alter table public.help_center_articles drop constraint if exists help_center_articles_organization_id_slug_key;
create unique index if not exists help_center_articles_project_slug_idx
  on public.help_center_articles (project_id, slug) where project_id is not null;
  ...
  and (
    ...
    or (
      ...
      and exists (
        ...
        where membership.organization_id = help_center_articles.organization_id
          and membership.user_id = auth.uid()
          and membership.status = 'active'
          and membership.role = any(help_center_articles.audience)
      )
    )
  )
  and exists (
    select 1 from public.projects project
    where project.id = help_center_articles.project_id
      and project.organization_id = help_center_articles.organization_id
...
as $$
  select jsonb_build_object(
    ...
    'body', p_article.body,
    'readingMinutes', p_article.reading_minutes,
    'version', p_article.version, 'updatedAt', p_article.updated_at
  );
$$;
...
begin
  ...
  return jsonb_build_object(
    ...
    'articles', coalesce((
      select jsonb_agg(public.help_center_article_json(article) order by article.sequence, article.title->>'en')
      from public.help_center_articles article
      where article.organization_id = v_org_id and article.project_id = v_project_id
        and (v_role = 'admin' or (article.status = 'published' and v_role = any(article.audience)))
    ), '[]'::jsonb)
  );
end;
$$;
...
begin
  ...
  if v_id is not null then
    ...
    update public.help_center_articles article set
      ...
      reading_minutes = greatest(coalesce((p_article->>'readingMinutes')::integer, article.reading_minutes), 1),
      title = p_article->'title', summary = p_article->'summary',
      tags = v_tags, body = p_article->'body',
    ...
    insert into public.help_center_articles (
      ...
      reading_minutes, title, summary, tags, body, created_by, updated_by
    ) values (
      ...
      greatest(coalesce((p_article->>'readingMinutes')::integer, 3), 1), p_article->'title', p_article->'summary',
      v_tags, p_article->'body', auth.uid(), auth.uid()
...
begin
  ...
  update public.help_center_articles article set
    status = p_status,
    version = article.version + 1, updated_by = auth.uid(), updated_at = clock_timestamp()
  where article.id = p_article_id and article.organization_id = v_org_id
    and article.project_id in (select project.id from public.projects project where project.organization_id = v_org_id and project.status = 'active')
    and p_expected_version is not null and article.version = p_expected_version
...
begin
  ...
  order by project.created_at limit 1;
  if v_project_id is null then raise exception 'ACTIVE_PROJECT_REQUIRED'; end if;
  if jsonb_typeof(p_order) <> 'array' or jsonb_array_length(p_order) = 0 then raise exception 'HELP_ARTICLE_ORDER_INVALID'; end if;
  for v_item in select value from jsonb_array_elements(p_order) loop
    update public.help_center_articles article set
      sequence = greatest((v_item->>'sequence')::integer, 0), version = article.version + 1,
      updated_by = auth.uid(), updated_at = clock_timestamp()
    where article.id = (v_item->>'id')::uuid and article.organization_id = v_org_id and article.project_id = v_project_id
      and nullif(v_item->>'version', '')::integer is not null and article.version = (v_item->>'version')::integer;
  ...
  values (v_org_id, auth.uid(), 'help_center_article', 'reordered', jsonb_build_object('projectId', v_project_id, 'order', p_order));
...
insert into public.help_center_articles (
  organization_id, project_id, slug, category, pmf_layer, audience, sequence, status, featured,
  reading_minutes, title, summary, tags, body
)
select
  ...
  4, jsonb_build_object('en', seed.title_en, 'zh', seed.title_zh),
  jsonb_build_object('en', seed.summary_en, 'zh', seed.summary_zh), seed.tags,
  jsonb_build_object(
